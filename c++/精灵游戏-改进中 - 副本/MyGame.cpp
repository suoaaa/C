...
void game_menu()        //开始菜单
{
    ...
    setTextSize(winwidth[config_code[1]]/16/10*7);    setTextBkColor(EMPTY);           //设置主标题文字格式
    paintText(winwidth[config_code[1]]/32*13,winhigh[config_code[1]]/6, "精灵游戏");

    setTextSize(winwidth[config_code[1]]/16/10*5);    setTextBkColor(EMPTY);            //设置三个副标题文字格式
    paintText(winwidth[config_code[1]]/32*14,winhigh[config_code[1]]/36*15, "开始游戏");
    paintText(winwidth[config_code[1]]/32*14,winhigh[config_code[1]]/36*21, "更改设置");
    paintText(winwidth[config_code[1]]/32*14,winhigh[config_code[1]]/36*27, "玩法说明");
...
{	static int n=0;
	...
	else
	{
		...
		{	n++;	cancelTimer(0);
        	...
        	beginPaint(); setTextSize(20);setTextBkColor(EMPTY); setTextColor(RED);paintText(140,90, "游戏失败");
			char *txtofscore=new char[10];sprintf(txtofscore,"%d",usr->getscore());
			paintText(140,120, "你的得分为：");paintText(265,120, txtofscore);endPaint();}
	}
}
void config_menu()        //显示当前设置
{
    ...
    rectangle(winwidth[config_code[1]]/32*8,    winhigh[config_code[1]]/18*7, winwidth[config_code[1]]/32*14, winhigh[config_code[1]]/18*9);           //第一行
    ...
    rectangle(winwidth[config_code[1]]/32*8,    winhigh[config_code[1]]/18*10, winwidth[config_code[1]]/32*14, winhigh[config_code[1]]/18*12);         //第二行
    ...
    rectangle(winwidth[config_code[1]]/32*8,    winhigh[config_code[1]]/18*13, winwidth[config_code[1]]/32*14, winhigh[config_code[1]]/18*15);         //第三行
    ...
    setTextSize(winwidth[config_code[1]]/16/10*7);    setTextBkColor(EMPTY);           //设置主标题文字格式
    paintText(winwidth[config_code[1]]/32*13,winhigh[config_code[1]]/6, "游戏设置");

    setTextSize(winwidth[config_code[1]]/16/10*5);    setTextBkColor(EMPTY);            //设置三个副标题文字格式
    paintText(  winwidth[config_code[1]]/32*9,  winhigh[config_code[1]]/36*15, "背景音乐");                 //第一行
    paintText(  winwidth[config_code[1]]/32*19.5,winhigh[config_code[1]]/36*15, "关");                 
    paintText(  winwidth[config_code[1]]/32*22.5,winhigh[config_code[1]]/36*15, "开");
    paintText(  winwidth[config_code[1]]/32*9,  winhigh[config_code[1]]/36*21, "窗口大小");                 //第二行
    paintText(  winwidth[config_code[1]]/32*18, winhigh[config_code[1]]/36*21, "小");
    paintText(  winwidth[config_code[1]]/32*21, winhigh[config_code[1]]/36*21, "中");
    paintText(  winwidth[config_code[1]]/32*24, winhigh[config_code[1]]/36*21, "大");
    paintText(  winwidth[config_code[1]]/32*9,  winhigh[config_code[1]]/36*27, "难度选择");                 //第三行
    paintText(  winwidth[config_code[1]]/32*18, winhigh[config_code[1]]/36*27, "易");
    paintText(  winwidth[config_code[1]]/32*21, winhigh[config_code[1]]/36*27, "中");
    paintText(  winwidth[config_code[1]]/32*24, winhigh[config_code[1]]/36*27, "难");
